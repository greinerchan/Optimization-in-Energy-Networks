...
%   and mutation to abtain the feasible solution and evaluate them
    for i = 1:1:pop
        curGenes = curPopulation.chromosomes(i).genes;
        curPopulation.chromosomes(i).genes = EDFeasible(mpc,curGenes);
    end
    for i = 1:1:pop
...
%%
for m = 1:1:pop
    [fitness, cost] = Fitness(mpc,curPopulation.chromosomes(m).genes);
    curPopulation.chromosomes(m).fitness = fitness;
    curPopulation.chromosomes(m).cost = cost;
end
